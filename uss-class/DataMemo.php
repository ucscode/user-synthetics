...
class DataMemo {
	...
	public function __construct( string $file ) {
		...
		*/
		...
		if( empty($file) ) throw new Exception( "Missing file path in argument 1"  );
		...
		if( empty($this->file) ) throw new Exception( "Invalid file path specified in argument 1. Try passing an absolute path instead" );
		
		
		# Configure datamemo
		
		if( !is_file($this->file) ) $this->memo = array();
	...
	protected function abspath( string $path ) {
		...
		if( $absolutes[0] === $parts[0] ) $absolutes = []; 
	...
	public function save() {
		...
		else return !!file_put_contents( $this->getFile(), $data, LOCK_EX );
	...
	public function __destruct() {
		$file = $this->getFile();
		if( $this->delete && file_exists( $file ) ) unlink( $file );
	}
