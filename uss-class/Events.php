...
class Events {
    ...
    protected static function sort_order($item, array $sort) {
        if( is_numeric($item) ) {
			 ...
			 * Throw Exception!
			 */
		} else throw new \Exception("Unhandled type: " . gettype($item));
    }
	...
	protected static function sortEvents( array &$eventList, ?bool $order ) {
		...
		if( !is_null($order) ) {
			...
			/** Reverse the order */
			if( !$order ) $sort = array_combine( array_keys($sort), array_reverse(array_values($sort)) );
			
			$convertType = function($value) {
				if( preg_match("/^\-?\d+/i", $value) ) $value = (float)$value;
				return $value;
			};
	...
	public static function exec( string $eventName, ?array $data = null, ?bool $sort = true, ?closure $master = null ) {
		...
		if( !array_key_exists($eventName, self::$events) ) return;
		...
		*/
		...
		foreach( $eventList as $key => $action ) {
			...
			*/
			...
			if( $run || is_null($run) ) call_user_func($action['callable'], $data ?? [], $eventName);
	...
	public static function addListener(string $eventNames, ?callable $callback, ?string $eid = null) {
		...
		self::splitEvents($eventNames, function($event) use($callback, $eid, $debug) {
			 ...
			 */
			if( is_null($eid) ) $eventList[] = $action;
			/**
			 * Else, if the eventID does not exist, add the event.
			 */ 
			else if( !array_key_exists($eid, $eventList) ) $eventList[ $eid ] = $action; 
	...
	public static function removeListener(string $eventNames, ?string $eid = null) {
		...
		self::splitEvents($eventNames, function($event) use($eid) {
			 ...
			 */
			if( is_null($eid) && !empty($eventList) ) array_pop($eventList);
			
			/**
			 * Else, remove the specified event
			 */
			else if( array_key_exists($eid, $eventList) ) unset($eventList[ $eid ]);
	...
	public static function getListener(string $eventName, ?string $eid = null) {
		...
		if( is_null($eid) ) {
			...
			if( $event ) return $event['callable'];
			
		} else if( array_key_exists($eid, $eventList) ) return $eventList[$eid]['callable'];
	...
	private static function splitEvents(string $eventNames, callable $func) {
		...
		foreach( $eventNames as $eventName ) $func( $eventName );
	}
	...
	public static function list( $priority = false, ?string $eventName = null ) {
		if( !$priority ) return array_keys(self::$events);
		$list = array();
